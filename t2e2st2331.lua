if IY_LOADED and not _G.IY_DEBUG == true then
	-- error("SlotsHub is already running!",0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'SlotsHub is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "SlotsHub FE v" .. currentVersion
...
PluginsHint.Text = "Download plugins, Not from the SlotsHub Discord, Moon Is a skid"
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- SlotsHub Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the SlotsHub support server.'}
...
addcmd('nohead',{'rhead','headless'},function(args, speaker)
	if sethidden then
		-- Full credit to Brembo.py @Discord
...
task.spawn(function()
	...
	minimizeHolder()
	if table.find({Enum.Platform.IOS, Enum.Platform.Android}, UserInputService:GetPlatform()) then notify("Unstable Device", "On mobile, SlotsHub may have issues or features that are not functioning correctly.") end
end)
